...
const testCases = [
  ...
  {
    specFile: 'spec/project/something/foo_spec.rb',
    codeFile: 'lib/project/something/foo.rb',
  },
  {
    specFile: 'spec/other_project/something_else/foo_spec.rb',
...
describe('specToCode', () => {
  testCases.forEach((testCase) => {
    it('gets the spec file for the code', () => {
      expect(resolver.specToCode(testCase.specFile, appsList)).toEqual(
        testCase.codeFile,
      );
    });
  });
...
describe('getRelated', () => {
  testCases.forEach((testCase) => {
    it('gets the correct related file', () => {
      expect(resolver.getRelated(testCase.codeFile, appsList)).toEqual(
        testCase.specFile,
      );
      expect(resolver.getRelated(testCase.specFile, appsList)).toEqual(
        testCase.codeFile,
      );
    });
  });
